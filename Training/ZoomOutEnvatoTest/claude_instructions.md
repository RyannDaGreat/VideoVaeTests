...
- Plan: **4-6 GPUs for training**, remaining GPUs for periodic inference
...
|   |   |-- conditions/
|   |   |-- reference_latents/
|-- configs/                        # Training YAML configs (generated by train.sh or prepare_dataset.py)
|   |-- zoom_out_ic_lora.yaml       # Main training config
|   |-- accelerate_ddp.yaml         # Accelerate config for multi-GPU
...
1. Sets `CUDA_VISIBLE_DEVICES` for training GPUs (e.g. `0,1,2,3,4,5` for 6 GPUs)
2. Generates `configs/zoom_out_ic_lora.yaml` from the template if not already present
3. Generates `configs/accelerate_ddp.yaml` for multi-GPU DDP if not already present
4. Launches training via accelerate:
   ```bash
   cd /root/CleanCode/Github/VideoVaeTests/LTX2/src/packages/ltx-trainer
   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 \
   uv run accelerate launch --config_file {path_to_accelerate_config} \
     scripts/train.py {path_to_training_config}
   ```

Training config key values (based on `ltx2_v2v_ic_lora.yaml` template):
```yaml
model:
...
optimization:
  learning_rate: 2e-5           # Conservative to be safe
  steps: 100000          # Long run - 100K iterations
  batch_size: 1
...
data:
  preprocessed_data_root: "{path_to_datasets}/.precomputed"

validation:
  # Use REAL captions from the test split (not made-up prompts)
  # Pick ~4-6 from the 50-video test set for the trainer's built-in validation
  prompts:
    - "{caption_from_test_video_1}"
    - "{caption_from_test_video_2}"
    - "{caption_from_test_video_3}"
    - "{caption_from_test_video_4}"
  reference_videos:
    - "{path_to_test_reference_video_1}"
    - "{path_to_test_reference_video_2}"
    - "{path_to_test_reference_video_3}"
    - "{path_to_test_reference_video_4}"
  video_dims: [512, 320, 121]  # Must match training resolution bucket [W, H, F]
  frame_rate: 24.0
  inference_steps: 30
  guidance_scale: 4.0
  interval: 500                 # Validate every checkpoint save
  include_reference_in_output: true
  videos_per_prompt: 1
  generate_audio: false

checkpoints:
  interval: 500                 # Save frequently (~every 10-30 min depending on step speed)
  keep_last_n: -1               # Keep ALL checkpoints - unlimited disk space
  precision: "bfloat16"
...
Accelerate DDP config for 6 GPUs:
```yaml
...
LTX-2 operates at **25 fps** by default. 121 frames at 25fps = **4.84 seconds** of video.
...
12. **Train with 4-6 GPUs**, leave remaining GPUs for inference
13. **Periodic inference** in a separate tmux pane - generates videos from test set every time
    a new checkpoint is saved (or every ~30 min)
