...
Usage:
    ...
    python transform.py /path/to/sample_dir --ref_out_path ref.mp4 --tgt_out_path tgt.mp4 --seed 0
"""
...
def process_pexels_sample(
    ...
    Args:
        ...
        seed: Random seed for reproducible temporal crops. If None, uses default randomness.
    """
    # Set seed for reproducible temporal crops (3 seeds per sample)
    if seed is not None:
        import random
        random.seed(seed)
        rp.np.random.seed(seed)
    ...
    # Resize videos to target dimensions (768x480)
    keyframes, raw_video = rp.resize_videos_to_hold(keyframes, raw_video, width=768)
    ...
    # Resize back to target dimensions (768x480)
