...
# 768x480 resolution, 121 frames (validation uses shortest bucket)
...
SHUFFLE="${SHUFFLE:-true}"  # Shuffle test set for random variety across GPUs (set to false for original order)
...
run() {
    ...
    uv run python scripts/inference.py \
        ...
        --reference-video "$REF" \
        --prompt "$CAPTION" \
        --height 480 --width 768 --num-frames 121 \
        --skip-audio \
