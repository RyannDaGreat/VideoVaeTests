"""Frame flicker transform: generates ref/tgt video pairs for IC LoRA training.

Reference = flickery video (nearest-keyframe replacement) with indicator bar.
...
def process_video(
    ...
    nn_indices = rp.quantize_to_nearest_values(range(num_frames), keyframe_indices)
    nn_frames = input_video[nn_indices]
    ...
    ref_out[:, -height:, -width:] = nn_frames
